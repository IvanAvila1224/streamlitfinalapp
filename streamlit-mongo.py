...
st.title("Compra Venta UV")
# Pull data from the collection.
# Uses st.cache_data to only rerun when the query changes or after 10 min.
@st.cache_data(ttl=600)
def getReactions(_db):
    items = _db.memes_reactions.find()
    items = list(items)  # make hashable for st.cache_data
    return items

def getComments(_db):
    items = _db.memes_comments.find()
    items = list(items)  # make hashable for st.cache_data
    return items

def getReactionsSummary(_db):
    items = _db.memes_reactions_sumary.find()
    items = list(items)  # make hashable for st.cache_data
    return items
...
st.sidebar.title("Jaime Ivan Avila Muñoz")
st.sidebar.write("s20006783")
st.sidebar.markdown("___")
st.sidebar.write("Visita la pagina:\n\n https://appfinalrelease-service-v2-ivanavila1224.cloud.okteto.net/")
st.sidebar.markdown("___")
